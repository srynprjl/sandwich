...
def category_exists(con:sqlite3.Connection ,id: int = None, shorthand: str = None):
  if(id is None and shorthand is None):
    ...
    sql = '''SELECT id, name, shorthand FROM categories WHERE shorthand={shorthand}'''
  else:
...
def add_category(con:sqlite3.Connection, name: str, shorthand: str, description:str =None):
...
def update_category(con: sqlite3.Connection,new_data: dict[str, str], id:int=None, shorthand:str=None):
  data = category_exists(con, id, shorthand)
  cur = con.cursor()
  sql_piece = ""
  for k,v in new_data:
